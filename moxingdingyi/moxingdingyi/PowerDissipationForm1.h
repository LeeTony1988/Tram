...
{
	//如果没有方案被选中就返回
	if (this->x_proposalsnumht->Keys->Count <= 0) {
	...
	int maxregionnum = int::Parse(maxregionnumdt->Rows[0]->ItemArray[0]->ToString());
	//按照最大的区间数创建Series,每个区间就是一个Series
	/*绘制chart控件*/
	this->chart1->Series->Clear();// ***********************************
	...
	for (int i = 0; i < maxregionnum; i++) {
		...
		int biaozhi = 0;
		for each (String ^ key in this->x_proposalsnumht->Keys)
		{
			ProposalData^ pp = safe_cast<ProposalData^>(this->x_proposalsnumht[key]);
			y_result_powerdissipation[biaozhi] = pp->regions[i]->powerDissipation;
			x_result_proposalsnum[biaozhi] = key->Trim();
			biaozhi++;
		}
	...
	//绘制echarts
	this->webBrowser1->Document->InvokeScript("ClearAll");
	for (int i = 0; i < maxregionnum; i++)//有regionnum个Series
	{
		...
		{
			ProposalData^ pp = safe_cast<ProposalData^>(this->x_proposalsnumht[key]);
			y_result_runtime[biaozhi] = pp->regions[i]->runningTime;
			x_result_proposalsnum[biaozhi] = key->Trim();
			biaozhi++;
		}
	...
	private:System::Void InitWebBrowser() {
		...
		this->webBrowser1->DocumentCompleted += gcnew System::Windows::Forms::WebBrowserDocumentCompletedEventHandler(this, &PowerDissipationForm::WebBrowserLoad);
		...
		this->webBrowser2->DocumentCompleted += gcnew System::Windows::Forms::WebBrowserDocumentCompletedEventHandler(this, &PowerDissipationForm::WebBrowserLoad);
		...
		this->webBrowser3->DocumentCompleted += gcnew System::Windows::Forms::WebBrowserDocumentCompletedEventHandler(this, &PowerDissipationForm::WebBrowserLoad);
		...
		this->webBrowser4->DocumentCompleted += gcnew System::Windows::Forms::WebBrowserDocumentCompletedEventHandler(this, &PowerDissipationForm::WebBrowserLoad);
	...
	private:System::Void WebBrowserLoad(System::Object^ sender, System::Windows::Forms::WebBrowserDocumentCompletedEventArgs^ e) {
		PaintChart();
	}
		//主窗体加载事件
...
private: System::Void toolStripMenuItem_CheckedChanged(System::Object^  sender, System::EventArgs^  e) {
	...
	if (item->Checked == true) {
		//方案被选中，加入到x_proposalsnumht中等待显示
		//创建方案对象
		ProposalData^ pp = gcnew ProposalData(item->Text, this->dateTimePicker1->Value.ToShortDateString());
		this->x_proposalsnumht[item->Text] = pp;
	}
	else if (item->Checked == false)
		//方案被剔除
		this->x_proposalsnumht->Remove(item->Text);
	PaintChart();
}
		 //快捷时段功耗显示
